...
def FA(x,y,cin):
    ...
    #single return, only logic gates, think as wires
    #sum finder
    #s1 and s2 -> gor(s1,s2)
    s1 = gand(gnot(x),gor(gand(gnot(y),cin),gand(y,gnot(cin))))
    s2 = gand(x,gor(gand(gnot(y),gnot(cin)),gand(y,cin)))
    s3 = gor(s1,s2) #sums combined
    #carry_out finder
    #c1 and c2 -> gor(c1,c2)
    c1 = gand(gor(gand(gnot(x),y), gand(x,gnot(y))),cin)
    c2 = gand(gand(x,y),gor(gnot(cin),cin))
    c3= gor(c1,c2) #carry_outs combined
    return(c3,s3)
...
def fourBitAdd(xxxx,yyyy):
    ...
    (e,zq) = FA(xxxx[1],yyyy[1],d)
    (f,zr) = FA(xxxx[0],yyyy[0],e)
    return (f,(zr,zq,zt,zo))
